...
门洞控制的正交异形房间已完成第一轮规则发现：L-01、L-03、L-04 A～E、L-05和多凹凸P-01样例确认了整房单一相位与主次区组合两类候选、独立窄带与连续异形砖的区别、突出带优先吸收、中央整砖与临墙非整砖原则，以及墙砖对缝只作为有上限的整体美观因素。第二绝对下限的准确数值、综合美观指标和墙砖缝DWG图元语义尚未冻结；DR1/DR2矩形核心与候选基座现已完成，但这些未冻结项仍阻止最终异形自动择优和墙砖对缝接入，也不启动M10。
...
- 旋转矩形和自定义 UCS
- 任意斜边、旋转网格、门块/动态块自动识别、异形房间门洞控制、柱、地漏和多外环
...
`EngineeringRectangularLayoutCalculator` 是独立于 AutoCAD 的 DR2 入口。它接收轴对齐矩形、砖宽/高和门洞所在墙及沿墙范围，分别计算门洞法向与沿墙方向：合法自然余量保持不动；`0 < r < 0.42T` 且至少有一块整砖时重分配为 `0.5T + 中间整砖 + (0.5T + r)`；恰好 `0.42T` 和整除尺寸不触发重分配。四面门洞共用同一旋转/镜像规则，居中时默认“北优先于南、西优先于东”，并提供一个只翻转等价沿墙分配的候选。

候选基座保留轴向边界带、分格线、按需生成的实际砖块轮廓、内部/边界分类、结构化诊断、淘汰原因，以及内部非整砖、过渡缝、边界非整砖、默认下限违规、相位重置、连续异形砖、首视线和关键对缝等原始指标输入。当前没有第二绝对下限常量，也没有综合美观权重或唯一评分。详细规则与自动验证记录见 [docs/door-controlled-rectangular-layout.md](docs/door-controlled-rectangular-layout.md)。

## DR3/DR4 AutoCAD 两点门洞交互与实机验收

`TILEDOORRECT` 的流程冻结为“矩形边界 → 砖宽/砖高 → 门洞第一点/第二点 → 门洞摘要 → 候选摘要与临时预览 → 接受/翻转/重选/取消”。两点必须在集中坐标公差内投影到同一面墙、投影后不同且不超出墙段；公差内越过墙端的数值会夹到墙端，超过公差则拒绝。

临时预览使用屏幕矢量，不创建数据库实体。取消、无效输入和重选都不会访问正式输出图层；居中门洞且 DR2 确实提供等价候选时才允许翻转。接受前再次检查 10,000 条上限，随后只在一个写事务中创建或复用 `TILE_LAYOUT_DOOR_RECT` 并写入所选候选。输入边界只读，插件不保存 DWG，成功写回可紧接一次 `U`/`UNDO` 整体撤销。

新增的点投影、消息和交互状态测试不引用 Autodesk 程序集。2026-07-24 锁定模式恢复、完整解决方案 Debug/Release 重建和两套 116/116 自动测试均通过；`TILE600`、`TILELAYOUT`、`TILEORTHO` 的既有回归继续通过。

2026-07-28，用户在 AutoCAD 2021 的新脱敏工作副本完成 DR4 一次性实机清单，步骤 1～12 全部通过。居中默认候选、翻转及回翻、取消/重选零写入、同点/异墙/超墙段拒绝、翻转后接受并写入 4 条分格线、紧接一次 `U` 整体撤销、10001 条在 10000 上限和图层创建前拒绝，以及三个既有命令兼容冒烟均符合预期。首次尝试因当前 AutoCAD 进程已加载旧的 `build/orthogonal-room/Debug` 同名同版本 DLL 而出现未知命令；文件锁核对定位后，用户关闭且不保存，启动新会话并加载 SHA-256 为 `0A4FCD106F52B9B1BDA4F82690891B447E5678C13C16BA4C2870628693F0D750` 的 DR3 DLL，随后整轮通过。最终关闭不保存，工作副本保持 31,890 字节，前后 SHA-256 均为 `646A3A7A22CF40E5EC0B9CF8621A17AFAB09BB27928772C05D9CB3F4202DDA75`；未通过项 0，未执行项 0。详细清单和证据见 [docs/door-controlled-rectangular-layout.md](docs/door-controlled-rectangular-layout.md)。
